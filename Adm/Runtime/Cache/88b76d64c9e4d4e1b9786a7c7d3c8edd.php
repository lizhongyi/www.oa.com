...
<?php if(ACTION_NAME == cateclass ): ?><div class="admin_nav_li"><a href="__APP__/Site/cateclass">职位列表</a></div>
<?php else: ?> 
<div class="admin_nav_li_hover"><a href="__APP__/Site/cateclass">职位列表</a></div><?php endif; ?>
...
删除顶级级分类将会自动删除此分类下的子分类。<br />
  ...
  <form id="form1" name="form1" method="post" action="__APP__/Site/industry_all_save">
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" id="list" class="link_lan"  >
    ...
    <tr>
	  ...
	  <?php if(is_array($district)): $i = 0; $__LIST__ = $district;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>

      <td   class="admin_list admin_list_first" >

      <input type="checkbox" name="id[]" value="<?php echo ($list["id"]); ?>" id="<?php echo ($list["id"]); ?>"  class="Bcheck"/>

	  <input name="save_id[]" type="hidden" value="<?php echo ($list["id"]); ?>" />

      <img src="__PUBLIC__/skin/images/left_e.gif" border="0" align="absmiddle"  id="<?php echo ($list["id"]); ?>" class="leftimg pointer"/>

	  <input name="categoryname[]" type="text"  value="<?php echo ($list["categoryname"]); ?>" class="input_text_150" style="color:#0066CC; font-weight:bold;  "/>

	  <span style="color:#CCCCCC">(id:<?php echo ($list["id"]); ?>)</span>	  </td>
      ...
      <td align="center"  class="admin_list admin_list_first">
	   ...
	   <input name="category_order[]" type="text"  value="<?php echo ($list["category_order"]); ?>" class="input_text_50"/>
      ...
      <td class="admin_list" style="text-align:center;">
			  ...
			  <a onClick="return confirm('你确定要删除吗？')" href="__APP__/Site/industry_del?id=<?php echo ($list["id"]); ?>">删除</a>
			...
			<!--小类 -->
			 ...
			 <?php if(is_array($list['sub'])): $i = 0; $__LIST__ = $list['sub'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$subs): $mod = ($i % 2 );++$i;?><tr class="Sclass_<?php echo ($list["id"]); ?>"  style="display:none" >

			  <td  class="admin_list"  style="padding-left:53px;">

			 <img src="__PUBLIC__/skin/images/cat_left1.gif" border="0" align="absmiddle" />

			  <input type="checkbox" name="id[]" value="<?php echo ($subs["id"]); ?>" class="Scheck_<?php echo ($subs["id"]); ?>" id="<?php echo ($subs["id"]); ?>"/>

			  <input name="save_id[]" type="hidden" value="<?php echo ($subs["id"]); ?>" />

			   <input name="categoryname[]" type="text"  value="<?php echo ($subs["categoryname"]); ?>" class="input_text_150"/>

			  <span style="color: #CCCCCC">(id:<?php echo ($subs["id"]); ?>)</span>			  </td>
			  ...
			  <input name="category_order[]" type="text"  value="<?php echo ($subs["category_order"]); ?>" class="input_text_50"/>
			  ...
			  <a onClick="return confirm('你确定要删除吗？')" href="__APP__/Site/industry_del?id=<?php echo ($subs["id"]); ?>">删除</a>

					 </td>

			</tr><?php endforeach; endif; else: echo "" ;endif; ?>

			<tr class="Sclass_<?php echo ($list["id"]); ?>" style="display:none" >
			  ...
			  <span class="AddCat" id="<?php echo ($list["id"]); ?>" style=" text-decoration:underline; cursor:pointer">增加分类</span>
